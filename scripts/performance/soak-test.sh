...
INITIAL_MEM=$(docker stats ticketero-api --no-stream --format "{{.MemUsage}}" | cut -d'/' -f1 | tr -d 'MiB ')
...
while [ $(date +%s) -lt $END_TIME ]; do
    ...
    CURRENT_MEM=$(docker stats ticketero-api --no-stream --format "{{.MemUsage}}" 2>/dev/null | cut -d'/' -f1 | tr -d 'MiB ')
...
# Final memory
FINAL_MEM=$(docker stats ticketero-api --no-stream --format "{{.MemUsage}}" | cut -d'/' -f1 | tr -d 'MiB ')
